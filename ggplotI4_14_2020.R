...
# mimic of underlying base graphics - doesn't need df like ggplot
...
my_histo <- function(x_var, fil_col = "coral"){
  qplot(x = x_var, color = I("black"), fill = I(fil_col))
}

my_histo(d$hwy)      
my_histo(d$hwy, "dodgerblue")

# basic density plot
qplot(x = d$hwy, geom = "density")

# basic scatter plot
qplot(x = d$displ, y = d$hwy,
      geom = c("smooth", "point"))
...
# basic boxplot
qplot(x = d$fl, y = d$cty, geom = "boxplot", fill = I("coral"))

# basic barplot ("long format")
qplot(x = d$fl, geom = "bar", fill = I("coral"))

# common mistake (need I(), otherwise ggpot thinks its a variable)
qplot(x = d$fl, geom = "bar", fill = "coral")
...
p1 + theme_classic(base_size = 9) # scales axes, labels, and markers
...
p1 <- ggplot(data = d, mapping = aes(x = displ, y = cty)) +
  ...
  labs(title = "My Graph Title Here",
       subtitle = "an extended subtitle that will print below title",
       x = "my x-axis label",
       y = "my y-axis label") +
  xlim(0, 4) +
  ylim(0, 20)
