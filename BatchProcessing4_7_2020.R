...
# ------------------------------
#
# need to create files for this exercise - normally you'd use your own real data
# -------------------------------
...
for(i in seq_len(file_n)){ # one for each file
  file_length <- sample(file_size[1]:file_size[2], size = 1)
  var_x <- runif(file_length) # random x
  var_y <- runif(file_length) # random y
  df <- data.frame(var_x, var_y) # bind into a data frame
  bad_vals <- rpois(n = 1, lambda = file_NA) # determine NA number
  df[sample(nrow(df), size = bad_vals), 1] <- NA # picks which rows are NA x
  df[sample(nrow(df), size = bad_vals), 2] <- NA # and y
  ...
  file_label <- paste(file_folder, 
                      ...
                      ".csv",
                      sep = "")
  # set up data file and incorporate timestamp and minimal metadata
...
# -------------------------------
# FUNCTION reg_stats
# description: fit linear model, extract model stats
# inputs: 2-column data frame (x, y)
...
for(i in seq_along(file_names)){
  data <- read.table(file = paste(file_folder, file_name[i], sep = ""),
                     sep = ",", header = TRUE)
  d_clean <- data[complete.cases(data),] # remove NAs
  
  . <- reg_stats(d_clean) # pull out regression stats from clean file
  stats_out[i,3:5] <- unlist(.) #output is list, this puts them in each col
}
