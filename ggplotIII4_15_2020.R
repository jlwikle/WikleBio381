...
# stacking with color transparency, adjust alpha which is color transparency 
# of each bar
...
# better to use position = fill for stacking with constant height
# aesthetic mapping for multiple groups in each bar
p1 <- ggplot(d, aes(x = drv, fill = fl)) +
...
# much better for a box plot (more information)
p2 <- ggplot(d, aes(x = fl, y = hwy, fill = fl)) +
  geom_boxplot()
print(p2)

# overlay raw data (for more information)
...
# saturation - intensity or vibrance of color
...
# Aesthetics
# attractive colors
# - large geoms (bars, boxplots) use light, pale colors
# - small geoms (points, lines) use dark, vibrant colors
# color palettes that are visible to those who are colorblind
...
# symbolic colors (heat = red, cool = blue, photosythesis/growth = green,
# oligotrophic = blue, infected = red)
# dyes or stains, or even colors of organisms

# discrete scales - distinct groups

# continuous scales - as in a heat map (gradient)
...
# use consistent color scheme for manuscript
...
my_r_cols <- c("red", "coral", "dodgerblue", "thistle", "lawngreen")
demoplot(my_r_cols, "pie")
